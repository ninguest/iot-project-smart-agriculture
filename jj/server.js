...
// WLED IS HERE
const WLED_PICO_IP = '192.168.193.163';
// 🌐 Turn WLED ON
app.post('/wled/on', async (req, res) => {
  try {
      const response = await axios.post(`http://${WLED_PICO_IP}:5000/wled/on`);
      res.send(response.data);
  } catch (error) {
      res.status(500).send("Error connecting to Pico WLED");
  }
});

// 🌐 Turn WLED OFF
app.post('/wled/off', async (req, res) => {
  try {
      const response = await axios.post(`http://${WLED_PICO_IP}:5000/wled/off`);
      res.send(response.data);
  } catch (error) {
      res.status(500).send("Error connecting to Pico WLED");
  }
...
// 🎨 Set Color
app.post('/wled/color', async (req, res) => {
  try {
      const { color } = req.body;
      const response = await axios.post(`http://${WLED_PICO_IP}:5000/wled/color?color=${color}`);
      res.send(response.data);
  } catch (error) {
...
// ✨ Set Effect
app.post('/wled/effect', async (req, res) => {
  try {
      const { effect } = req.body;
      const response = await axios.post(`http://${WLED_PICO_IP}:5000/wled/effect?effect=${effect}`);
      res.send(response.data);
  } catch (error) {
      res.status(500).send("Error setting effect");
  }
});

// 🔆 Set Brightness
app.post('/wled/brightness', async (req, res) => {
  try {
      const { brightness } = req.body;
      const response = await axios.post(`http://${WLED_PICO_IP}:5000/wled/brightness?brightness=${brightness}`);
      res.send(response.data);
  } catch (error) {
